...
for (i in 1:ncol(dat)) {
  #select just those samples that have taxon i
  x = dat[dat[, i] > 0,] 
  #get column sums. This is how often taxon i cooccurs with taxa in j in this set
  #make this the row in the matrix
  adjmat[, i] = colSums(x)
}
...
write.csv(adjmat, file=paste("./Adj_matrix_weighted_", abrv_filename, sep=""))
}

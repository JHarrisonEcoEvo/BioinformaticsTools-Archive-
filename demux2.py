...
for line in forwardreads:
    ...
    if regex.match('^@.+:.+:', line) is not None:
        ...
        pattern = "(.+)(" + forwardPrimer + "){s<=3}"
        #print(pattern)
        fb = regex.findall(pattern , read)
        #print(read)
        ...
        if not fb:
            ...
            if fb[0][0] not in key_f:
                levdist = []
                levkeys = []
                for k in key_f:
                    levkeys.append(k)
                    levdist.append(Levenshtein.distance(fb[0][0], k))
        ...
        #Note that we are at the same line here as we are in the forward reads
        header_r = reversereads.readline()
        read_r = reversereads.readline()
        line3_r = reversereads.readline()
        quality_r = reversereads.readline()

        pattern_r = "(.+)(" + reversePrimer + "){s<=3}"

        rb = regex.findall(pattern_r, read_r)
        #print(rb,"rb")
        if not rb:
            rb = "unknown"
        else:
            if rb[0][0] not in key_r:

                # rpattern = "(" + rb[0][0] + "){s<=3, i<=1, d<=1}"
                # print(key_r[17], "key17")
                # print(rpattern, "rpattern")
                # print(regex.findall(rpattern , key_r[17]), "findall")
                # newlist = list(filter(rpattern.search, key_r))
                # print(newlist, "out")
                ...
                for k in key_r:
                    levkeys.append(k)
                    levdist.append(Levenshtein.distance(rb[0][0], k))
